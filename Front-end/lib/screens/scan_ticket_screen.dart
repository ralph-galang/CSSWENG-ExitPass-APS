...
class _ScanTicketScreenState extends State<ScanTicketScreen> {
  ...
  Future<void> _handleEnterTicket() async {
    ...
    } on ParkingSessionAlreadyPaidException catch (e) {
      if (!mounted) return;
      setState(() {
        _isLookingUp = false;
        _errorMessage = e.message;
      });
    } on TicketAlreadyRecordedException catch (e) {
      if (!mounted) return;
      setState(() {
        _isLookingUp = false;
        _errorMessage = e.message;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      bottomNavigationBar: AppBottomNavBar(
        currentIndex: 1,
        onTap: (i) => AppBottomNavBar.navigateToTab(context, '/tickets', i),
      ),
      body: SafeArea(
        top: false,
        child: SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
              // Visual placeholder only -- no camera package wired in. Add
              // mobile_scanner and call lookupSession(qrPayload: ...) the same way ticketNumber is used below.
              const AspectRatio(
                aspectRatio: 1,
                child: DottedBorderContainer(
                  child: Center(
                    child: Column(
                      ...
                      children: [
                        AppIcon(svg: AppIcons.qrFrame, size: 64, color: AppColors.gray900),
                        SizedBox(height: 16),
                        Padding(
